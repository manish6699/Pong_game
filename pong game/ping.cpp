...
/* Main ball */
Ball ball = { 0.5, 0, 0, 3.0, 0.10, 0.10 };
...
static void printscore()
{
	...
	sprintf_s(text, "Level %d - Score %d - Lives %d", level, player_score, player_lives);
	/* Yellow color */
	...
	if (ball.y <= ball_maxd)
	{
		...
		if (ball.x <= bar.x - bar.w or ball.x >= bar.x + bar.w) // Bar missing
		{
			...
			if (player_lives == 0)
			{
				...
				isGameEnd = true;
				isGameOver = true;
				strcpy_s(finaltext, "Game Over! You lost all three balls");
			}
		}
		else // Score
		{
			...
			else if (player_score == level_c)
			{
				...
				strcpy_s(finaltext, "Congratulation! Top score recorded");
				isGameEnd = true;
			}
		}
	}
...
{
	if(btn==GLUT_LEFT_BUTTON && state==GLUT_DOWN && bar.x > bar_maxl)
		bar.x -= 1.0;
	else if(btn == GLUT_RIGHT_BUTTON && state == GLUT_DOWN	&& bar.x < bar_maxr)
		bar.x += 1.0;

}

void ballcolor(int key)
{
	switch (key)
	{
	case 1:
...
int main(int argc, char** argv)
{
	...
	glutMouseFunc(mouse);
	menutest=glutCreateMenu(ballcolor);
	glutAddMenuEntry("red", 1);
	glutAddMenuEntry("green", 2);
	glutAddMenuEntry("blue", 3);
	ballsize = glutCreateMenu(ballradius);
	glutAddMenuEntry("min", 1);
	glutAddMenuEntry("normal", 2);
	glutAddMenuEntry("max", 3);
	glutCreateMenu(mainmenu);
	glutAddSubMenu("ball color",menutest);
